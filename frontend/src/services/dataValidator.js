...
// // import { decodeHtmlEntities, findChefFromWishText } from './supabase';
...
// // // Nettoyage optimise pour la reconnaissance des noms et dossiers PDF
...
// // // Reconnaissance intelligente d un etudiant a partir du chemin ou nom de dossier PDF
...
// import * as XLSX from 'xlsx';
// import { decodeHtmlEntities, findChefFromWishText, normalizeSpecialiteKey } from './supabase';
...
// // ============================================================================
// // 4. VALIDATION DU QUESTIONNAIRE APTITUDES / APPETENCES (BLINDEE SQL)
// // ============================================================================
// export const validateCompetencesScores = (rows, type = 'aptitudes', etudiantsList = [], referentielCompetences = []) => {
//   const anomalies = [];
//   const cleanPayload = [];
...
//   // Cartographie dynamique robuste : garantit TOUJOURS une vraie colonne SQL (11 colonnes officielles)
//   const competenceColMapping = referentielCompetences.map((comp, cIdx) => {
//     // Normalise le code pour correspondre aux vraies colonnes PostgreSQL
//     const sqlColName = normalizeSpecialiteKey(comp.code);
//     const targetMoodle = (comp.intitule_moodle || '').toLowerCase().trim();
//     const targetLabel = (comp.label || '').toLowerCase().trim();

//     // Recherche dans l entete CSV
//     const matchingIndices = [];
//     firstRow.forEach((headerVal, hIdx) => {
//       const hStr = cleanCellString(headerVal).toLowerCase().trim();
//       if (!hStr) return;

//       const matchesMoodle = targetMoodle && hStr === targetMoodle;
//       const matchesCode = normalizeSpecialiteKey(hStr) === sqlColName;
//       const matchesLabel = targetLabel && hStr.includes(targetLabel);

//       if (matchesMoodle || matchesCode || matchesLabel) {
//         matchingIndices.push(hIdx);
//       }
//     });

//     // 1ere occurrence = Aptitudes (Bloc 1), 2eme occurrence = Appetences (Bloc 2)
//     let selectedColIdx = -1;
//     if (matchingIndices.length > 1) {
//       selectedColIdx = type === 'aptitudes' ? matchingIndices[0] : matchingIndices[1];
//     } else if (matchingIndices.length === 1) {
//       selectedColIdx = matchingIndices[0];
//     } else {
//       selectedColIdx = startOffset + cIdx;
//     }
...
//     const scores = { adresse_email: email, etudiant_id: student.id };
//     competenceColMapping.forEach(({ sqlColumn, colIdx }) => {
//       const cellVal = r[colIdx] !== undefined ? r[colIdx] : '';
//       const parsed = parseInt(cleanCellString(cellVal), 10);
//       // Cle SQL officielle stricte
//       scores[sqlColumn] = !isNaN(parsed) && parsed >= 0 ? parsed : 0;
//     });
...
// ============================================================================
// 5. VALIDATION DES DOCUMENTS (CV et Lettres de motivation) - CORRIGEE
// ============================================================================
export const validateDocumentsList = (filesList, etudiantsList = []) => {
  const anomalies = [];
  const cleanPayload = []; // Nom de variable unifie
  ...
  return {
    status: bloquants > 0 ? 'BLOQUANT' : alertes > 0 ? 'AVERTISSEMENT' : 'CONFORME',
    stats: { total: filesArray.length, valides: cleanPayload.length, bloquants, alertes },
    anomalies,
    cleanPayload,
  };
};